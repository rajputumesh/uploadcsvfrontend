...
const Batting = () =>{
    ...
    const filterbattings = battings.filter(batting => {
        return batting.runs.toLocaleLowerCase().includes(search.toLocaleLowerCase());
    });
    
    const batting = filterbattings.map((data)=>{
        const {_id, bf, fs, ground, inns, opposition, matchNumber,dismissal ,mins, pos, result, runs, ss, sr, startDateAscending} = data;
        return(
            <tr key={_id}>
                <td>{runs}</td>
                <td>{mins}</td>
